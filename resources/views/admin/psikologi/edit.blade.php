...
@section('content')
    ...
    <div class="bg-white shadow-lg rounded-lg p-6 md:p-8">
        <form action="{{ route('admin.psikologi.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            {{-- Hidden input untuk mengirimkan kode pelamar terkait --}}
            @if(isset($pelamarKode) && $pelamarKode)
                <input type="hidden" name="pelamar_kode_terkait" value="{{ $pelamarKode }}">
                {{-- Anda mungkin perlu cara untuk mendapatkan nama pelamar berdasarkan kode ini --}}
                {{-- Jika nama pelamar dibutuhkan untuk disimpan di tabel psikologi --}}
                {{-- <input type="hidden" name="nama_pelamar_input" value="Nama Pelamar Otomatis dari Kode {{ $pelamarKode }}"> --}}
            @else
                 {{-- Jika pelamar_kode tidak ada, mungkin perlu input manual untuk kode dan nama pelamar --}}
                 {{-- <div class="mb-4">
                    <label for="kode_pelamar_input_manual" class="block text-sm font-medium text-gray-700">Kode Pelamar (Manual)</label>
                    <input type="text" name="pelamar_kode_terkait" id="kode_pelamar_input_manual" class="form-input mt-1 block w-full" placeholder="Mis: D-123">
                </div>
                 <div class="mb-4">
                    <label for="nama_pelamar_input_manual" class="block text-sm font-medium text-gray-700">Nama Pelamar (Manual)</label>
                    <input type="text" name="nama_pelamar_input" id="nama_pelamar_input_manual" class="form-input mt-1 block w-full" placeholder="Nama Lengkap Pelamar">
                </div> --}}
            @endif
            ...
            <div class="space-y-6">
                ...
                <div>
                    <label for="dokumen_tes" class="block text-sm font-medium text-gray-700 mb-1">
                        Dokumen Tes Psikologi {{-- Opsional, sesuaikan jika wajib --}}
                    </label>
                    <div class="mt-1">
            ...
            <div class="mt-8 pt-6 border-t border-gray-200 flex justify-end space-x-3">
                {{-- Tombol Batal mengarah ke daftar psikologi dengan filter pelamar_kode jika ada --}}
                <a href="{{ isset($pelamarKode) && $pelamarKode ? route('admin.psikologi.index', ['pelamar_kode' => $pelamarKode]) : route('admin.psikologi.index') }}" 
