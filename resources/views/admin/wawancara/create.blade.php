...
@section('content')
    ...
    <div class="bg-white shadow-lg rounded-lg p-6 md:p-8">
        <form action="{{ route('admin.wawancara.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @if(isset($pelamarKode) && $pelamarKode)
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <select id="jabatan_pewawancara" name="jabatan_pewawancara" 
                        ...
                        @foreach($jabatanOptions ?? [] as $jabatan)
                            <option value="{{ $jabatan }}" {{ old('jabatan_pewawancara') == $jabatan ? 'selected' : '' }}>{{ $jabatan }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div>
                    <label for="dokumen_wawancara" class="block text-sm font-medium text-gray-700 mb-1">
                        Dokumen Wawancara {{-- Opsional, jadi tidak ada bintang merah --}}
                    </label>
                    <div class="mt-1">
                ...
                <div>
                    ...
                    <div class="mt-1">
                        <textarea id="kesimpulan_wawancara" name="kesimpulan_wawancara" rows="8" 
                                  class="form-textarea block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm py-2 px-3">{{ old('kesimpulan_wawancara') }}</textarea>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select id="rekomendasi_wawancara" name="rekomendasi_wawancara" 
                        ...
                        <option value="rekomendasikan" {{ old('rekomendasi_wawancara') == 'rekomendasikan' ? 'selected' : '' }}>Rekomendasikan</option>
                        <option value="tidak_rekomendasikan" {{ old('rekomendasi_wawancara') == 'tidak_rekomendasikan' ? 'selected' : '' }}>Tidak Rekomendasikan</option>
                    </select>
                </div>
            </div>
