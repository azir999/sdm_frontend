...
@section('content')
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div class="p-4 flex flex-col sm:flex-row justify-between items-center space-y-3 sm:space-y-0 border-b border-gray-200">
            {{-- Kolom kiri: Search dan Filter Dropdown --}}
                ...
                <div class="relative w-full sm:w-auto">
                    ...
                    <div id="filterWawancaraMenu" class="origin-top-left absolute left-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none hidden z-50" role="menu" aria-orientation="vertical" aria-labelledby="filterWawancaraButton">
                        <div class="py-1" role="none">
                            ...
                            <a href="#" data-filter-value="gagal" class="filter-item-wawancara text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100" role="menuitem">Gagal</a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="w-full sm:w-auto">
                <a href="{{ route('admin.wawancara.create') }}" class="w-full sm:w-auto bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-3 rounded-md shadow-sm inline-flex items-center justify-center text-sm">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-gray-800 uppercase tracking-wider">Pangkat</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200 text-sm text-gray-700">
                    ...
                    <tr>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $wawancara->kode_pelamar }}</td>
                        <td class="px-4 py-3 whitespace-nowrap font-medium">{{ $wawancara->nama_pelamar }}</td>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $wawancara->nama_pewawancara }}</td>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $wawancara->pangkat_pewawancara }}</td>
                        <td class="px-4 py-3 whitespace-nowrap">{{ $wawancara->tanggal }}</td>
    ...
    if (filterButton && filterMenu) {
        ...
        filterMenu.querySelectorAll('.filter-item-wawancara').forEach(item => { 
            item.addEventListener('click', function(event) {
                ...
                console.log('Filter Data Wawancara dipilih:', filterValue);
