...
@section('title', 'Update Data Penerimaan Dosen')

@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-text-dark">Update Data Penerimaan Dosen</h1>
        <p class="text-sm text-text-muted">Sistem Informasi Sumber Daya Manusia</p>
    </div>

    <div class="bg-white shadow-lg rounded-lg p-6 md:p-8">
        @if(isset($dosen))
            <form action="{{ route('admin.penerimaan.dosen.update', $dosen->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-y-6"> 
                    <div>
                        ...
                        <input type="text" name="kode" id="kode" value="{{ old('kode', $dosen->kode ?? '') }}" 
                    ...
                    <div>
                        ...
                        <input type="text" name="nik" id="nik" value="{{ old('nik', $dosen->nik ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                     ...
                     <div>
                        ...
                        <select id="jenis_kelamin" name="jenis_kelamin" 
                            ...
                            @foreach($jenisKelaminOptions ?? ['Laki Laki', 'Perempuan'] as $option)
                                <option value="{{ $option }}" {{ old('jenis_kelamin', $dosen->jenis_kelamin ?? '') == $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="golongan_darah" class="block text-sm font-medium text-gray-700 mb-1">Golongan Darah</label>
                        <input type="text" name="golongan_darah" id="golongan_darah" value="{{ old('golongan_darah', $dosen->golongan_darah ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label for="agama" class="block text-sm font-medium text-gray-700 mb-1">Agama</label>
                        <input type="text" name="agama" id="agama" value="{{ old('agama', $dosen->agama ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label for="nomor_hp" class="block text-sm font-medium text-gray-700 mb-1">Nomor HP</label>
                        <input type="tel" name="nomor_hp" id="nomor_hp" value="{{ old('nomor_hp', $dosen->nomor_hp ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label for="fakultas" class="block text-sm font-medium text-gray-700 mb-1">Fakultas</label>
                        <input type="text" name="fakultas" id="fakultas" value="{{ old('fakultas', $dosen->fakultas ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label for="program_studi" class="block text-sm font-medium text-gray-700 mb-1">Program Studi</label>
                        <input type="text" name="program_studi" id="program_studi" value="{{ old('program_studi', $dosen->program_studi ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                     ...
                     <div>
                        ...
                        <input type="text" name="ikatan_kerja" id="ikatan_kerja" value="{{ old('ikatan_kerja', $dosen->ikatan_kerja ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                     <div>
                        <label for="tanggal_mulai_bertugas" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Mulai Bertugas</label>
                        <input type="date" name="tanggal_mulai_bertugas" id="tanggal_mulai_bertugas" value="{{ old('tanggal_mulai_bertugas', $dosen->tanggal_mulai_bertugas ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label for="pendidikan_tertinggi" class="block text-sm font-medium text-gray-700 mb-1">Pendidikan Tertinggi</label>
                         <select id="pendidikan_tertinggi" name="pendidikan_tertinggi" 
                            ...
                            @foreach($pendidikanOptions ?? [] as $option)
                                <option value="{{ $option }}" {{ old('pendidikan_tertinggi', $dosen->pendidikan_tertinggi ?? '') == $option ? 'selected' : '' }}>{{ $option }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label for="jabatan_akademik" class="block text-sm font-medium text-gray-700 mb-1">Jabatan Akademik</label>
                        <input type="text" name="jabatan_akademik" id="jabatan_akademik" value="{{ old('jabatan_akademik', $dosen->jabatan_akademik ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        <label for="golongan_dosen" class="block text-sm font-medium text-gray-700 mb-1">Golongan Dosen</label>
                        <input type="text" name="golongan_dosen" id="golongan_dosen" value="{{ old('golongan_dosen', $dosen->golongan_dosen ?? '') }}" 
                               class="form-input mt-1 block w-full border-gray-300 rounded-md shadow-sm py-2 px-3 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                    </div>
                    <div>
                        ...
                        <select id="status_aktivasi" name="status_aktivasi" 
                            ...
                            @foreach($statusAktivasiOptions ?? ['aktif', 'nonaktif'] as $option)
                                <option value="{{ $option }}" {{ old('status_aktivasi', $dosen->status_aktivasi ?? '') == $option ? 'selected' : '' }}>{{ ucfirst($option) }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        @if(isset($dosen->foto_dosen_filename)) <p class="text-xs text-gray-500 mt-1">File saat ini: {{ $dosen->foto_dosen_filename }}. Kosongkan jika tidak ingin mengubah.</p> @endif
                    </div>
                    <div>
                        ...
                        <input type="file" name="dokumen_penerimaan" id="dokumen_penerimaan" 
                               class="form-input mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 border border-gray-300">
                         @if(isset($dosen->dokumen_filename)) <p class="text-xs text-gray-500 mt-1">File saat ini: {{ $dosen->dokumen_filename }}. Kosongkan jika tidak ingin mengubah.</p> @endif
                    </div>
        ...
        @else
            <p class="text-center text-gray-500">Data Dosen tidak ditemukan.</p>
